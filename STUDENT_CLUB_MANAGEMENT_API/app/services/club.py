...
def get_clubs_service(db: Session, current_user: UserModel, search: str | None = None):
    query = (
        db.query(ClubModel)
        .join(
            ClubMemberModel,
    ...
    if search:
        ...
        query = query.filter(ClubModel.name.ilike(f"%{search}%"))

    return query.all()

def get_club_detail_service(db: Session, club_id: int, current_user: UserModel):
    club = (
        db.query(ClubModel)
        .join(
            ClubMemberModel,
